...
- 调度检查点
    ...
    - logic：
        - 需要调度时：切换上下文；否则维持原进程
        - 常见的检查点：
            - 进程主动让出（等待IO、sleep、运行结束）
            - 时间片定时器
            - 强制中断
            - 内核态回到用户态时
    - 经验：
        - 时间片不是绝对的，只要内核在检查调度时发现需要调度，就会进行调度。这也导致 fork 绝不一定是父进程优先运行
...
## 一些函数/方法
- wait
    - 阻塞并等待自己的子进程结束。无法捕捉到其他进程的子进程
    - 子进程的子进程不是子进程，无法wait
    - 孤儿进程：父进程先终止了，OS会把子进程发给 init 进程
...
- cpu大多是intel的，而 intel 一开始用段，后来加了层页，所以原本用页的linux也不得不加层段了
- 线性地址
    - 段页式体制下的段物理地址/页虚拟地址
- 流程
...
fork, exec, wait
- fork 从调用行开始复制一份上下文和代码段，创建到子进程中继续运行
- exec 替换为新程序，但如果报错了就换回来
