...
- 线程的资源实际上大部分是共享的进程资源的，比如全局变量、虚拟内存啥的
- 一个进程内有多个线程
- 线程的优缺点
    - 优点：
        - 并发
...
- 线程切换
    - 基本上只需要切换堆栈、寄存器值就行了，内存都是同一片，不需要换
    - 如果不是同一个进程里的，逻辑和资源消耗就和切换进程一样
- 线程的实现有三种
    - 用户线程实现
        - 完全通过program模拟实现线程功能，包括时间片、调度器、生命周期啥的，因此用户线程完全不关心进程的功能，只会被进程当成普通program该咋跑咋跑
        - TCB保存在用户空间
        - 因为线程只需要切换堆栈、PC和寄存器就好，不需要任何内核态功能，因此速度非常快
        - 多个用户线程对应着一个内核线程(或者对于无线程的os来说是进程)
        - 有趣的是，常见的用户线程库有POSIX，之前也见到过
        - 不支持内核线程的os也能用用户线程
        - 缺点：
            - 如果一个用户线程的任务需要进入内核态且阻塞了，所有同一进程内的用户线程都会一起进入阻塞状态
            - 如果一个用户线程在运行，事实上也代表进程正在运行这个线程，如果线程自己不让出，就只有操作系统能打断运行中的任务
            - 由于操作系统，或者说进程并不知道用户线程的存在，分时间片不会考虑用户线程，而是只考虑进程，因此同一进程中的用户线程就只能瓜分进程分到的那一点时间片，相比内核线程短不少，执行比较慢
    - 内核线程实现
        - 操作系统，或者说内核管理的线程
        - TCB储存在操作系统，或者说内核空间中
        - 一个用户线程对应一个内核线程
        - 一个线程阻塞不会阻塞其他线程
        - 每个线程的时间片更多了，因为os调度注意到多线程了
        - 缺点：
            - 系统开销更大了
    - 轻量级进程 (Light-weight Process)
        - 一种介乎内核线程和用户线程之间的抽象概念，更诡异的是内核线程LWP用户线程可以同时存在：内核线程 -> LWP -> 用户线程
        - 它的存在意义在于提供一种机制，使得用户线程能够更高效地利用内核线程的资源，同时解决一些用户线程和内核线程各自的缺点。
        - 内核线程 1:1 LWP
        - LWP对用户线程的支持有三种模式
        - 三种模式
            - 1 : 1
