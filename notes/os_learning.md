...
- 线程控制块 Thread Control Block：
    ...
    - 注意线程本身是在进程内的，所有线程本身也是在**用户空间**的，只是TCB不一定在用户空间而已
- 线程切换
...
- 线程的实现有三种
    - 用户线程
        ...
        - TCB保存在用户空间，背后的调度只能由内核通过进程PCB决定
        - 只需要在用户空间切换堆栈、PC和寄存器就好，不需要任何内核态功能，因此速度非常快
        - 有趣的是，常见的用户线程库有POSIX，之前也见到过
        - 不支持内核线程的os也能用用户线程
        - 缺点：
            - 一个用户线程的任务阻塞了，所有同一进程内的用户线程都会一起进入阻塞状态
            - 线程间无法打断，只能主动让出（因为打断是内核功能，但TCB不在内核）
